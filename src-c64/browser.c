...
static void mainLoop(void)
{
    ...
    {
        gotoxy(DIR1X, DIR1Y);
        cputs("Error: InitialCardridge not detected.\n");
        gotoxy(DIR1X, DIR1Y+2);
        waitKey();
        return;
    }
        ...
        switch (c)
          {
            ...
            case CH_ENTER:
                    if (dir->no_of_elements)
                    {
                        ...
                        if (current->type == FILE_DIR || current->type == FILE_D64)
                        {
...
static void execute(DirElement *elem)
{
    ic_load_and_run_file(elem); // should not return
}
...
static void printNameSize(DirElement *element)
{
    ...
    {
      cprintf("%-20s DIR ", element->name);
    }
    else if (element->type == FILE_UNKNOWN)
    {
        textcolor(COLOR_GRAY1);
        cprintf("%-20s -  ", element->name);
    }
    else
    {
        uint32_t size = element->size;
        if (element->type == FILE_D64)
        {
            textcolor(COLOR_GREEN);
        }
        else if (element->type != FILE_PRG)
        {
        ...
        if (size >= 1024)
        {
            ...
            {
                size >>= 10;
                cprintf("%-20s %3luM", element->name, size);
            }
            else
            {
                cprintf("%-20s %3luk", element->name, size);
            }
        }
        else
        {
            cprintf("%-20s %3lub", element->name, element->size);
        }
    }
}
